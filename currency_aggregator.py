...
def parse_line(line: list[str]) -> tuple[Decimal, str] | None:
    if len(line) == 2: # COMMENT - Check to ensure the line is correctly formatted with exactly two parts (amount and currency). If not, we skip it.
        ...
        print(f"Skipping malformed line length: expected 2 parts, got '{len(line)}'", file=sys.stderr)
        return None
        ...
        with open(filepath, 'r') as f:
            ...
            for line in f:
                ...
                if result:
                    ...
                    totals[currency] += amount # COMMENT - defaultdict starts at 0.00 for a new key, otherwise adds amount
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("Error: Missing file argument. Usage: python3 script.py <filename>", file=sys.stderr)
        sys.exit(1)
    else:
